...
(function (factory) {
	...
	function CropAvatar($element, cb) {
		this.cb = cb;
		this.$container    = $('.avatar-view');
		this.$avatarInput  = $element;
	...
	CropAvatar.prototype = {
		...
		addListener: function () {
			this.$avatarInput.on('change', $.proxy(this.modalShow, this));
			this.$avatarForm.on('submit', $.proxy(this.submit, this));
			this.$avatarModal.on('shown.bs.modal', $.proxy(this.change, this));
			this.$avatarModal.on('hide.bs.modal', $.proxy(this.stopCropper, this));
		},
		// 初始化预览
		initPreview: function () {
			this.$avatarPreview.empty().html('<img>');
		},
		modalShow: function () {
		...
		startCropper: function () {
			...
			if (_this.active) {
				_this.$img.cropper('replace', this.url);
			} else {
				_this.$img = $('<img src="' + this.url + '">');
				this.$avatarWrapper.empty().html(this.$img);
				_this.$img.cropper({
					aspectRatio: 1,
					strict: false,
					crop: function() {
						var imgurl = _this.$img.cropper('getCroppedCanvas', {
							width: 200,
							height: 200
						}).toDataURL('image/jpeg', .7)
						_this.$avatarData.val(imgurl);
						_this.$avatarPreview.find('img').attr('src', imgurl);
					}
				});
				this.active = true;
			}
		...
		submitDone: function (data) {
			if (data.code === '0000') {
				...
				VUE.$Message.success(data.message);
				this.cb && this.cb(data);
			} else {
				VUE.$Message.warning(data.message);
				// VUE.$Message..alert('Failed to response');
			}
		},
		...
		$.ajax({
			...
			success: function(o) {
				if (!$('#' + id).length) {
					...
					$('body>.avatar-modal').attr('id', id);
				}
				new CropAvatar(_this, cb);
			}
		})
	};
});
...
if (!$('head #u_cropper').length) {
	$('head').append('<link id="u_cropper" rel="stylesheet" href="/js/util/cropper/cropper.css">');
}